...
# 读取 HTSeq 计数数据
counts_L0h <- read.table("Arabi/cufflinks_output/At-L0h_counts.txt", header=FALSE, row.names=1)
counts_L2h <- read.table("Arabi/cufflinks_output/At-L2h_counts.txt", header=FALSE, row.names=1)
counts_L24h <- read.table("Arabi/cufflinks_output/At-L24h_counts.txt", header=FALSE, row.names=1)
counts_L168h <- read.table("Arabi/cufflinks_output/At-L168h_counts.txt", header=FALSE, row.names=1)
...
# 读取基因表达文件
fpkm_data <- read.csv("Arabi/cufflinks_output/modified_expressed_genes.csv")
...
if ("log2FoldChange" %in% colnames(results)) {
  ...
  write.csv(cold_responsive_genes, "Arabi/cufflinks_output/cold_responsive_genes.csv", row.names=TRUE)
} else {
...
if ("log2FoldChange" %in% colnames(results)) {
  ...
  write.csv(non_responsive_genes, "Arabi/cufflinks_output/non_responsive_genes.csv", row.names=FALSE)
} else {
  stop("log2FoldChange 列未找到，请检查 DESeq2 分析输出")
}
