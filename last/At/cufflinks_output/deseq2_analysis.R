...
# 读取 HTSeq 计数数据
counts_files <- list.files(path="last/At/cufflinks_output/", pattern="*_counts.txt", full.names=TRUE)
...
# 读取表达基因文件
expressed_genes <- read.csv("last/At/cufflinks_output/expressed_genes.csv", header=TRUE, row.names=NULL)
...
# 输出冷响应基因和不响应基因的ID
write.csv(cold_responsive_genes, "last/At/cold_responsive_genes.csv", row.names = FALSE, quote = FALSE)
write.csv(nonresponsive_genes, "last/At/nonresponsive_genes.csv", row.names = FALSE, quote = FALSE)
